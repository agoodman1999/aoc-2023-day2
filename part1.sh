...
for line in "${file_lines[@]}"; do
	...
	if [[ "$largest_red_count" -gt "$allowed_red_count" ]] || [[ "$largest_green_count" -gt "$allowed_green_count" ]] || [[ "$largest_blue_count" -gt "$allowed_blue_count" ]]; then
		echo "Game $game_number: not possible"
	else
		echo "Game $game_number: possible"
		allowed_game_id_sum=$((allowed_game_id_sum + game_number))
	fi
